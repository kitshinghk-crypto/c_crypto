...
void pkcs1_15_sha256_sig(uint8_t* s, const uint8_t* m, const uint8_t* n, const uint8_t* d, size_t mlen){
    ...
    //print_hex(em, WORD_LENGTH);
    mont_exp(em,d16,n16,WORD_LENGTH*8-1,WORD_LENGTH);
    for(int i=0; i<WORD_LENGTH; i++){
        s[i]=em[i]&0xff;
    }
}