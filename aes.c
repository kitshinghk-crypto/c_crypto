...
int init_key_schedule(uint8_t* roundkey, const uint8_t* key){
    ...
    for (uint8_t w=4; w<44; w++){
        ...
        //SubWord
        if(w%4==0){
            *(roundkey+round*16+offset+0)=g_aes_sbox[*(roundkey+round*16+offset+0)];
            *(roundkey+round*16+offset+1)=g_aes_sbox[*(roundkey+round*16+offset+1)];
            *(roundkey+round*16+offset+2)=g_aes_sbox[*(roundkey+round*16+offset+2)];
            *(roundkey+round*16+offset+3)=g_aes_sbox[*(roundkey+round*16+offset+3)];
        }
        //XOR
